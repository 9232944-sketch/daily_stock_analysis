...
strip_invalid_packaged_signature() {
  ...
  echo "WARNING: removing invalid pre-existing signature from PyInstaller artifact: ${packaged_file}"
  codesign --remove-signature "${packaged_file}"

  signature_details="$(codesign -d "${packaged_file}" 2>&1 || true)"
  if [[ "${signature_details}" != *"code object is not signed at all"* ]]; then
    echo "ERROR: failed to clear invalid signature immediately after PyInstaller packaging: ${packaged_file}"
    echo "${signature_details}" >&2
    exit 1
  fi
}
...
main() {
  ...
  while IFS= read -r packaged_file; do
    if file -b "${packaged_file}" | grep -q "Mach-O"; then
      strip_invalid_packaged_signature "${packaged_file}"
    fi
