...
"""Stable adapter for DSA's built-in screening engine.

DSA should depend on this module instead of screening pipeline internals,
...
from src.services.screening import REFERENCE_PROJECT, REFERENCE_REVISION, __version__
from src.services.screening.pipeline import screen as run_screen
from src.services.screening.strategy import list_strategies as load_strategies
...
def list_strategies(context: Dict[str, Any] | None = None) -> List[Dict[str, Any]]:
    """List enabled built-in strategies using a DSA-stable shape."""
    return [
        {
            "id": item.name,
