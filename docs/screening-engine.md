...
- 默认本地 `scorecard` 会覆盖完整短名单，保证所有可能进入近分轮换的候选使用同一最终评分口径；`dsa` 与 `external_http` 等远程后置分析仍严格遵守 `POST_ANALYSIS_MAX_PICKS` 上限，状态按实际提交候选记录，外部响应中的超限代码不会改写未提交候选；启用远程分析时轮换只会在已完成相同分析的候选之间发生。
