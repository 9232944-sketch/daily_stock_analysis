...
分享图片用于把个股分析和市场复盘转换为适合社交平台传播的 1080px 长图。个股和大盘使用两套独立的信息结构，但共用 DSA 品牌、仓库标识 `ZhuLinsen/daily_stock_analysis` 和风险声明。GitHub 区不放二维码；Web 与桌面端分享图默认展示仓库内置小红书二维码，部署配置可替换二维码和账号信息。
...
小红书品牌使用以下可选配置。全部留空时仅展示仓库内置二维码；配置任一自定义值后仅使用这组自定义品牌信息，不再补默认昵称或二维码，避免把自定义账号与默认二维码混合：
...
- 小红书 URL、昵称、ID 和二维码路径可由运行时配置覆盖；全部留空时仅使用仓库内置二维码，默认不展示数字 ID，也不注入源码内置账号。GitHub 固定展示仓库标识 `ZhuLinsen/daily_stock_analysis`，不生成二维码。
