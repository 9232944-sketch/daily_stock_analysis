...
- [改进] 热点成分股并行获取东方财富与同花顺数据，并按固定数据源优先级合并；AkShare 调用复用 DSA 的可终止子进程 timeout，同花顺 HTTP 调用设置 connect/read timeout，并以进程级并发槽限制活跃任务、在返回前回收 worker；题材详情可按需复用 DSA 原生搜索服务补充安全且带链接的近期消息，并在本地压缩摘要以避免额外 LLM 等待与降级提示；显式搜索增强不写入服务端共享缓存或 Web 页面缓存
