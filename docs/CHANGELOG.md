...
- [修复] 选股尾部轮换以分析器输入顺序为权威并保留并列分候选顺序；热点消息增强分别追加 `route` 与原始 `timeline`，同键搜索只允许缓存 owner 启动供应商链，子进程启动或清理失败也会释放全局容量；热点默认 provider 的正数超时配置覆盖整次调用并传递剩余硬截止，关闭外层预算仍保留单源安全上限，主动新闻搜索始终保留可终止子进程截止
